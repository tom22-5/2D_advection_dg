...
\\ 
= \quad &\int^{1}_{-1} \int^{1}_{-1} \Phi_i(\zeta_0, \zeta_1) [2 (x_1 - x_0)^{-1} a_0(z_0, z_1, t) \partial_{\zeta_0} \Phi_j(\zeta_0, \zeta_1) + 2 (y_1 - y_0)^{-1}
\\ 
\quad &a_1(z_0, z_1, t) \partial_{\zeta_1} \Phi_j(\zeta_0, \zeta_1)]  \frac{1}{4} (x_1 - x_0) (y_1 - y_0) d\zeta_0 d\zeta_1
\\
= \quad &\sum_{q_0} \sum_{q_1} w_{q_0} w_{q_1} \phi_{i_0}(x_{q_0}) \phi_{i_1}(x_{q_1}) [\frac{1}{2} (y_1 - y_0) a_0(z_{q_0}, z_{q_1}, t) \phi'_{j_0}(x_{q_0}) \phi_{j_1}(x_{q_1}) 
\\
\quad &+ \frac{1}{2} (x_1 - x_0) a_1(z_{q_0}, z_{q_1}, t) \phi_{j_0}(x_{q_0}) \phi'_{j_1}(x_{q_1})].
\end{align*}

We write $z_k = x(\zeta_k)$. The face terms are slightly more difficult to derive as in two dimensions we have four faces that are either interior and face other elements or that are part of the domain boundary. This is why for a specific derivation, we need to choose an explicit flux and face. In this example, we choose an upwind flux for information flow between elements, i.e. 
\begin{align*}
\widehat{au}
=
\begin{cases}
...
\end{cases}
\end{align*}
and focus on the left face. As this is one-dimensional, we will only need the second $x_1(\zeta)$ part of the transformation, that concerns the $x_1$-axis. The corresponding determinant of the inverse Jacobian is simply $(y_1 - y_0)$ for our tensor-product grid. Generally it holds that 
\begin{align*}
&\int_{\partial \Omega_e} \widehat{au}(\Phi_i^R(z), \Phi_i^L(z)) \Phi_j(z) dz
\\
= \quad &\int^1_{-1} (y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(\zeta) \widehat{au}(\Phi^R_i(\zeta), \Phi^L_i(\zeta)) d\zeta
\\
\approx \quad &\sum_q w_q (y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(x_q) \widehat{au}(u^R_i \phi^R_{i_0}(-1) \phi^R_{i_1}(x_1(x_q)), u^L_i \phi^L_{i_0}(-1) \phi^L_{i_1}(x_1(x_q))).
\end{align*}

The value of $\widehat{au}(u^R_i \phi^R_{i_0}(-1) \phi^R_{i_1}(x_1(x_q)), u^L_i \phi^L_{i_0}(-1) \phi^L_{i_1}(x_1(x_q)))$ depends on the following cases. 
