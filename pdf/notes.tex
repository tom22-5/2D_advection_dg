...
Now insert basis functions of order $p$, i.e.
\begin{align*}
...
\end{align}
We consider a tensor-product mesh with $N = N_x \times N_y$ rectangular elements. Then $x: \Omega_{\textnormal{ref}} \rightarrow \Omega_e$ maps from the reference element $\Omega_{\textnormal{ref}} = [-1,1]^2$ to the actual element $\Omega_e = [x_0, x_1] \times [y_0, y_1]$ and can be explicitly written as the affine transformation
\begin{align*}
...
\\ 
= \quad &\int^{x_1}_{x_0} \int^{y_1}_{y_0} \Phi_j(z_0, z_1) \Phi_i(z_0, z_1) dz_0 dz_1 
\\
= \quad &\int^1_{-1} \int^1_{-1} \Phi_j(\zeta_1, \zeta_2)  \Phi_i(\zeta_1, \zeta_2) \|\operatorname{det}(J_e)\| d\zeta_1 d\zeta_2,
\end{align*}
...
M_{ji} = \quad &\int_{\Omega_e} \Phi_j(z) \Phi_i(z) dz
\\ 
\approx \quad &\sum_{q_0} \sum_{q_1} w_{q_0} w_{q_1} \phi_{i_0}(x_{q_0}) \phi_{i_1}(x_{q_1}) \phi_{j_0}(x_{q_0}) \phi_{j_1}(x_{q_1}) \frac{1}{4} (x_1 - x_0) (y_1 - y_0)
\end{align*}
when inserting the tensor-product form $\Phi_i = \phi_{i_0} \phi_{i_1}$ and exploiting $\|\operatorname{det}(J_e)\| = \frac{1}{4} (x_1 - x_0) (y_1 - y_0)$.

Similarly, we can derive an expression for the volume matrix and calculate
\begin{align*}
B_{ji} = \quad &\int_{\Omega_e} \Phi_i(z) (a(z, t) \cdot \nabla \Phi_j(z)) dz
\\
= \quad &\int^{x_1}_{x_0} \int^{y_1}_{y_0} \Phi_i(z_0, z_1) (a(z_0, z_1, t) \cdot \nabla \Phi_j(z_0, z_1)) dz_0 dz_1 
\\
= \quad &\int^{1}_{-1} \int^{1}_{-1} \Phi_i(\zeta_0, \zeta_1) (a(z_0, z_1, t) \cdot J^{-1}_e \nabla \Phi_j(\zeta_0, \zeta_1)) \|\operatorname{det}(J_e)\| d\zeta_0 d\zeta_1 
\\ 
= \quad &\int^{1}_{-1} \int^{1}_{-1} \Phi_i(\zeta_0, \zeta_1) [2 (x_1 - x_0)^{-1} a_0(x(\zeta), t) \partial_{\zeta_0} \Phi_j(\zeta_0, \zeta_1) + 2 (y_1 - y_0)^{-1}
\\ 
\quad &a_1(x(\zeta), t) \partial_{\zeta_1} \Phi_j(\zeta_0, \zeta_1)]  \frac{1}{4} (x_1 - x_0) (y_1 - y_0) d\zeta_0 d\zeta_1
\\
= \quad &\sum_{q_0} \sum_{q_1} w_{q_0} w_{q_1} \phi_{i_0}(x_{q_0}) \phi_{i_1}(x_{q_1}) [\frac{1}{2} (y_1 - y_0) a_0(x(x_q), t) \phi'_{j_0}(x_{q_0}) \phi_{j_1}(x_{q_1}) 
\\
\quad &+ \frac{1}{2} (x_1 - x_0) a_1(x(x_q), t) \phi_{j_0}(x_{q_0}) \phi'_{j_1}(x_{q_1})].
\end{align*}
...
\\
= \quad &\int^1_{-1} \frac{1}{2}(y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(\zeta) \widehat{au}(\Phi^R_i(\zeta), \Phi^L_i(\zeta)) d\zeta
\\
\approx \quad &\sum_q w_q \frac{1}{2}(y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(x_q) \widehat{au}(u^R_i \phi^R_{i_0}(-1) \phi^R_{i_1}(x_1(x_q)), u^L_i \phi^L_{i_0}(-1) \phi^L_{i_1}(x_1(x_q))),
\end{align*}
where $w_q, x_q$ are one-dimensional quadrature weights and points.

The value of $\widehat{au}(u^R_i \phi^R_{i_0}(-1) \phi^R_{i_1}(x_1(x_q)), u^L_i \phi^L_{i_0}(-1) \phi^L_{i_1}(x_1(x_q)))$ depends on the following cases, we write the first and second component of our reference to element mapping as $x_0$ and $x_1$. 

\textbf{Case 1A: Inflow Boundary.} \\
\begin{align*}
{G_{\textnormal{bound}}}_j \approx \quad &\sum_q w_q \frac{1}{2} (y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(x_q) (a(x_0(-1), x_1(x_q), t) \cdot n) u(x_0(-1), x_1(x_q), t)
\end{align*}

\textbf{Case 1B: Outflow Boundary.} \\
\begin{align*}
G_{ji^R} \approx \quad &\sum_q w_q \frac{1}{2} (y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(x_q) (a(x_0(-1), x_1(x_q), t) \cdot n) \phi^R_{i_0}(-1) \phi^R_{i_1}(x_q) u^R_i
\end{align*}

\textbf{Case 2A: Inflow Interior Face.} \\
\begin{align*}
G_{ji^L} \approx \quad &\sum_q w_q \frac{1}{2} (y_1 - y_0) \phi_{j_0}(-1) \phi_{j_1}(x_q) (a(x_0(-1), x_1(x_q), t) \cdot n) \phi^L_{i_0}(-1) \phi^L_{i_1}(x_q) u^L_i
\end{align*}
...
Notice that the outflow cases 1B and 2B yield the same formula. Now that we have set up the semi-discretization in space, we can turn our attention to time integration. We can evolve \eqref{eq:ode} in time by applying a Runge--Kutta method. In our experiments, we set $N_x = 10, N_y = 10$ and $p=3$. We choose Gauss-Legendre quadrature with $N_q = p + 1$ 1D quadrature nodes for Lagrange elements
\begin{align*}
...
\end{align*}
with the corresponding initial and boundary Dirichlet conditions. Figure \ref{fig:exact} shows different snapshots of the exact solution. It is evident that the solution at final time and initial time is the same for $T=1$. For our experiments, we chose $T=0.1$ and measure the error in the Euclidean norm.

\begin{figure}[h!]
    \centering
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../plots/snapshot-0.00.png}
        \caption{$t = 0.00$}
    \end{subfigure}
    \hfill
        \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../plots/snapshot-0.25.png}
        \caption{$t = 0.25$}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../plots/snapshot-0.50.png}
        \caption{$t = 0.50$}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../plots/snapshot-0.75.png}
        \caption{$t = 0.75$}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../plots/snapshot-1.00.png}
        \caption{$t = 1.00$}
    \end{subfigure}
...
Figure \ref{fig:explicitresults} shows the convergence behaviour for the different explicit integrators of order 1 to 4 listed in figure \ref{fig:butcherExplicit}. As expected, the higher the order of the method, the more accurate is the solution with respect to the step size. It is also no surprise that the runtime of higher order methods is larger for a fixed step size as shown in figure \ref{fig:explicitruntime}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_explicit_convergence.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_explicit_runtime.png}
    \caption{Runtime of explicit Runge--Kutta methods with 1 to 4 stages.}
    \label{fig:explicitruntime}
\end{figure}

A DIRK scheme is an implicit Runge--Kutta method with $a_{ij} = 0$ for all $j>i$. SDIRK methods further fulfill $a_{ii} = a_{jj}$ for all $i,j \leq s$, where $s$ denotes the number of stages. For the implicit SDIRK schemes~\cite{kennedy2016diagonally} with 1 to 4 stages from figure \ref{fig:butcherImplicit}, we get the convergence results illustrated in figure \ref{fig:implicitresults}. Clearly, the implicit methods deliver more accurate results for larger step sizes. However, figure \ref{fig:implicitruntime} shows that this comes at the expense of a longer runtime.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{array}
        \]
        \caption{SDIRK2--NCS23 (order 3), $\gamma = (3 + \sqrt{3}) / 6$~\cite[p. 27]{kennedy2016diagonally}}
    \end{subfigure}
    ...
    \begin{subfigure}{\textwidth}
        ...
        \end{array}
        \]
        \caption{SDIRK3--NC34 (order 4), $\gamma = \frac{3 + 2 \sqrt{3} cos(\Pi / 18)}{6}$~\cite[p. 27]{kennedy2016diagonally}}
    \end{subfigure}
    \hfill
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_implicit_convergence.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_implicit_runtime.png}
    \caption{Runtime of implicit Runge--Kutta methods with 1 to 4 stages.}
    \label{fig:implicitruntime}
\end{figure}

In order to address this issue and speed up the implicit integrators, we would like to use the matrix-free tensor-product preconditioners of Pazner~\cite{pazner2018approximate} and Diosady~\cite{diosady2017tensor} for the linear system arising during implicit time integration. We begin with implementing the former for matrix-based code. Notice that we do not exploit the efficiency advantages of a matrix-free tensor-product implementation yet.
...
\begin{align*}
(M - h a_{jj} (B - G)) U_j = M U_0 - h a_{jj} G_{\textnormal{bound}}(t_j) + h \sum^{j-1}_{l=1} a_{jl} [(B - G) U - G_{\textnormal{bound}}(t_l)]
\end{align*}
at each stage $j$. Notice that the right-hand side can be explicitly computed, such that we arrive at a linear system. Notice that for a SDIRK method, it holds that $a_{jj} = a_{ll}$ for all $j,l$ such that we always have the same operator on the left, thus can always use the same preconditioner and precompute it only once.

Pazner~\cite{pazner2018approximate} proposes to use an element-wise rank-2 Kronecker SVD to do so, i.e. for $A_e = M_e - h a_{jj} (G_e - B_e)$ we estimate
\begin{align}
\label{eq:ksvd}
A_e \approx \sum^r_{j=1} A_j \otimes B_j,
\end{align}
which can be computed via an SVD of a shuffled version $\tilde{A}_e$ of $A_e$. The full SVD's cost scales cubicly in the size of the matrix to be estimated, see \cite[Ch. 8.6]{G13}. Luckily, there are ways to avoid this, namely the Lanczos SVD and the Randomized SVD~\cite{T23}. Both reduce the runtime to near squared complexity and deliver near-optimal accuracy with high probability. In his paper, Pazner decides to use the Lanczos algorithm. For $r=2$, the approximation \eqref{eq:ksvd} can be inverted effiently by analytical means, see \cite[Ch. 4]{pazner2018approximate}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_precon_convergence.png}
...
In terms of runtime, we cannot recognise a significant speed-up as demonstrated in figure \ref{fig:paznertime}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_precon_runtime.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/rk_precon_iterations.png}
...
We also visualize these results in table \ref{tab:iterations}. Notice that we display the average number of iterations per stage. For the 4-stage ESDIRK method with an explicit first stage, we thus divide by 4 even though we only have to solve an implicit system for 3 stages. This is why GMRES appears to perform much better here.
